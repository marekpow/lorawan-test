...
static int8_t r_reg(Si7021_registers_t reg)
{
  ...
  if(HAL_OK != HAL_I2C_Mem_Read(&hi2c3, I2C_ADDR, cmd, 1, data, 1, 10000))
    return -1;
  else
    return 0;
}
...
int8_t r_firmware_rev_Si7021()
{
  ...
  if(HAL_OK != HAL_I2C_Master_Transmit(&hi2c3, I2C_ADDR, (uint8_t*)(&cmd), 2, 10000))
    return -1;

  if(HAL_OK != HAL_I2C_Master_Receive(&hi2c3, I2C_ADDR, &data, 1, 10000))
    return -1;
